...
public class ExDao extends CpDao {
	...
	public Long obterProximoNumeroNup(final ExDocumento doc, Long anoEmissao)
			throws SQLException {
		Query query = em().createNamedQuery("obterProximoNumeroNup");
		query.setParameter("anoEmissao", anoEmissao);
		return (Long) query.getSingleResult();
	}
...
//			return (ExProtocolo) query.getSingleResult();
	...
	public void preencherParametros(final ExMobilDaoFiltro flt,
			final Query query) {
...
//			query.setParameter("ultMovIdEstadoDoc", flt.getUltMovIdEstadoDoc());
		...
		if (flt.getNup() != null && !(flt.getNup().equals(""))) {
			query.setParameter("nup", flt.getNup());
		}
	...
	public List consultarPorFiltroOtimizado(final ExMobilDaoFiltro flt,
		...
		Query query = em().createQuery(
				montaQueryConsultaporFiltroLocal(flt, false));
	...
	public Integer consultarQuantidadePorFiltroOtimizado(
		...
		long tempoIni = System.nanoTime();
		IMontadorQuery montadorQuery = carregarPlugin();
		String s = montaQueryConsultaporFiltroLocal(flt, true);
...
//		long tempoIni = System.nanoTime();
	...
	public String montaQueryConsultaporFiltroLocal(final ExMobilDaoFiltro flt, boolean apenasCount) {
